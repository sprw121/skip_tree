typedef struct skip_tree_node {
	...
	struct skip_tree_node* d_par;			// The node's skip parent, dyanmic
	struct skip_tree_node* d_lchild;		// The node's skip left child, dynamic
	struct skip_tree_node* d_rchild;		// The node's skip right child, dyanmic
	void * data;
} skip_tree_node;
